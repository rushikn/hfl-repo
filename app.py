...
def results_to_natural_language(results, user_query):
    ...
    # System prompt to reduce typos and ensure clear output
    system_prompt = (
    "You are a precise business analytics assistant specialized in summarizing SQL query results for reporting. "
    "Your job is to generate grammatically correct, concise, and contextually accurate summaries strictly based on the result data. "
    "Follow these rules:\n"
    "- Do not introduce any new terms or assumptions by your own .\n"
    "- Only refer to what's present in the SQL result.\n"
    "- Focus on sales, growth, comparisons, or totals as asked by the user.\n"
    "- Use a formal tone without casual phrases.\n"
    "- Do not add explanations unless the user explicitly requested it."
)

    prompt_text = (
        f"The user asked: \"{user_query}\"\n\n"
        f"The SQL query returned the following result:\n\n{results_str}\n\n"
        "Based on the user's question and the query result, provide a short, clear summary."
        + (
            "\n- If the result involves a comparison or multiple categories, summarize it using a markdown table with appropriate headers."
            if is_comparison_query else
            "\n- Do not include units or additional commentary unless explicitly requested by the user."
    )
    + "\n\nFinal Summary:"
    )
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            temperature=0.2,
            top_p=1.0,
            frequency_penalty=0.0,  # Prevent repetitive phrases
            presence_penalty=0.0,  # Prevent introducing irrelevant concepts  # Lower temp = fewer creative errors
        )
